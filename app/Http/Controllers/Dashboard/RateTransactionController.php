...
use App\Http\Services\PaymentReceiptService;
...
class RateTransactionController extends Controller
{
    ...
    public function getCurrentBill(Request $request)
    {
        try {
            ...
            if ($ratepayer->entity_id !== null) {
               ...
               $pendingDemands = ClusterCurrentDemand::where('ratepayer_id', $ratepayer->id)
                  ->where('is_active', true)
                  // ->whereRaw('ifnull(demand,0) > ifnull(payment,0)')
                  ->whereRaw('ifnull(total_demand,0) - ifnull(payment,0) > 0')
                  ->whereRaw('(bill_month + (bill_year * 12)) <= (MONTH(CURRENT_DATE) + (YEAR(CURRENT_DATE) * 12))')
   ...
   //  }
    ...
    public function postWhatappPayment(Request $request)
    {
      try {
        $receiptService = new ReceiptService();
        ...
        $validatedData = $validator->validated();
        $validatedData['tc_id'] = 1;
        $validatedData['remarks'] = 'Whats app Bot payment';
        $validatedData['longitude'] = '0.0';
        $validatedData['latitude'] = '0.0';
        $validatedData['payment_mode'] = 'WHATSAPP';

        $retFlag = $paymentReceiptService->extractRatepayerDetails($validatedData);
        if ($retFlag == false) {
            return format_response(
                'Invalid Ratepayer',
                null,
        ...
        $retFlag = $paymentReceiptService->postPayment();
        if ($retFlag == false) {
            return format_response(
        ...
        $receiptData = $receiptService->generateReceipt($paymentReceiptService->paymentData);
        $encodedPdf = base64_encode($receiptData['content']);
        return format_response(
            'success',
            $encodedPdf,

...
class AdminDashboardController extends Controller
{
   public function getTransactionDetails(Request $request)
   {
       ...
       if (!empty($validatedData['zone'])) {
           $query->whereHas('cluster', function($q) use ($validatedData) {
               $q->where('name', $validatedData['zone']);
           });
       }
   ...
   private function getMonthlyTransactionOverview()
   {
       return CurrentTransaction::select(
           ...
           DB::raw('SUM(CASE WHEN event_type = "PAYMENT" THEN payment.amount ELSE 0 END) as amount')
       )
   ...
   {
       return Cluster::select('name as area')
   ...
   //     return Alert::where('is_active', 1)

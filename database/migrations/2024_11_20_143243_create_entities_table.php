...
return new class extends Migration
{
    ...
    public function up(): void
    {
      Schema::create('entities', function (Blueprint $table) {
         ...
         $table->foreignId('zone_id')->constrained('payment_zones');
         $table->foreignId('subcategory_id')->constrained('sub_categories');
         $table->foreignId('verify_user_id')->constrained('users');
         $table->foreignId('apply_tc_id')->constrained('users');
         $table->bigInteger('lastpayment_id')->unsigned();
      
         $table->string('holding_no', 255)->unique();
         $table->string('consumer_no', 255)->unique();
         ...
         $table->enum('usage_type', ['Household', 'Commercial'])->default('Household')->notNullable(); // Type of entity
         $table->enum('status', ['verified', 'un-verified', 'suspened', 'closed'])->default('un-verified')->notNullable();
